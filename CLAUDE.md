...
- SQLite (один файл БД); проектировать без SQLite-специфики, чтобы при необходимости уйти на Postgres
...
4. SQLite Row не сериализуется в JSON — перед `tojson` делать `[dict(r) for r in rows]`.

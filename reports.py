...
def summary_cards():
    return {
        ...
        'defects_overdue': query_db("SELECT COUNT(*) as c FROM defects WHERE due_date < to_char(now(),'YYYY-MM-DD') AND status NOT IN ('closed','verified')", one=True)['c'],
        'defects_closed': query_db("SELECT COUNT(*) as c FROM defects WHERE status IN ('closed','verified')", one=True)['c'],
        'substages_overdue': query_db("SELECT COUNT(*) as c FROM substages WHERE plan_end_date < to_char(now(),'YYYY-MM-DD') AND status NOT IN ('done','closed','approved')", one=True)['c'],
